...
# ─── Deprecated: legacy R-GCN encoder ──────────────────────────────
# These classes exist only to load checkpoints trained with the old
# separate RGCNBasisGNN encoder. New code should use:
#   SkipLastGNN(conv_type='RGCN', num_relations=N, num_bases=B)

def _legacy_extract_relation_ids(data, num_edges, num_relations):
    ...
    def forward(self, data):
        if self.feat_preprocess is not None:
            if not getattr(data, "preprocessed", False):
                ...
                data.preprocessed = True
        x, edge_index, batch = data.node_feature, data.edge_index, data.batch
        relation_ids = _legacy_extract_relation_ids(data, edge_index.size(1), self.num_relations)
        x = self.pre_mp(x)
        all_emb = x.unsqueeze(1)
        emb = x
